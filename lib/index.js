...
class FileUploader {
  ...
  upload({url, file, csp, maxFileSizeForChunking}) {
    ...
    if (!csp) {
      throw "Cloud service provider(csp) value required to process to request, support are azure, aws, oci"
    }
    ...
    switch (csp.toLowerCase()) {
      ...
      case "aws":
        this.uploaderInstance = new AwsUploader(url, file, maxFileSizeForChunking, this.emit);
        break;
